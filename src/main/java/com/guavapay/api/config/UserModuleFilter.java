...
@Component
public class UserModuleFilter extends AbstractGatewayFilterFactory<UserModuleFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (Objects.equals(clientId, clientUser)) return chain.filter(exchange.mutate().request(request).build());
            response.setStatusCode(HttpStatus.FORBIDDEN);
            return response.setComplete();
        };
    }
